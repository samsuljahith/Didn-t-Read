...
function providerNeedsApiKey(settings) {
  return settings.providerId !== 'chrome';
}
...
async function analyzeText(text, ctx) {
  const needsKey = providerNeedsApiKey(ctx.settings);
  const apiKey = needsKey ? await getApiKey() : '';
  ...
  if (tokens <= SAFE_INPUT_TOKENS) {
    ...
    summary = await callWithRetry(
      () =>
        generateStructuredSummary(apiKey, ctx.settings, buildFullPrompt(doc, ctx.settings), ctx.signal),
      ctx.signal,
      2,
    );
  } else {
    summary = await summarizeDocument(doc, apiKey, ctx.settings, ctx.onProgress, ctx.signal);
  }

  return groundSummary(summary, text);
}
