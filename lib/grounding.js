...
// PRIORITY_KEYS is defined in validate-summary.js (loaded before this file in the service worker).
...
function phraseIsGrounded(haystackNorm, phrase) {
  ...
  const words = p.split(' ').filter((w) => w.length > 2 && !GROUNDING_STOP_WORDS.has(w));
  if (words.length < 2) {
    return false;
  }
...
function groundSummary(summary, sourceText) {
  ...
  return {
    ...
    keyPoints: groundStringList(summary.keyPoints, haystackNorm),
    riskyClauses: groundStringList(summary.riskyClauses, haystackNorm),
    dataCollection: groundStringList(summary.dataCollection, haystackNorm),
    thirdPartySharing: groundStringList(summary.thirdPartySharing, haystackNorm),
    subscriptionTerms: groundStringList(summary.subscriptionTerms, haystackNorm),
    cancellationRefund: groundStringList(summary.cancellationRefund, haystackNorm),
    userObligations: groundStringList(summary.userObligations, haystackNorm),
    legalClauses: groundStringList(summary.legalClauses, haystackNorm),
  };
}
 ...
 * @param {string} haystackNorm
 */
function groundStringList(items, haystackNorm) {
  if (!Array.isArray(items)) {
    return items ?? [];
  }
  return items.filter((item) => phraseIsGrounded(haystackNorm, String(item)));
}
 ...
 * @param {string} sourceText
 */
function getPriorityGroundingFlags(priorities, sourceText) {
  if (!sourceText?.trim()) {
    return {};
  }
  const haystackNorm = normalizeForMatch(sourceText);
  ...
  for (const key of PRIORITY_KEYS) {
    const item = priorities[key];
    if (!item?.answer || item.status === 'unclear') {
      flags[key] = false;
      continue;
    }
    flags[key] = phraseIsGrounded(haystackNorm, item.answer);
  }
  return flags;
}
