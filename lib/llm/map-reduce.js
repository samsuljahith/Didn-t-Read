  ...
  const runOne = async (chunk) => {
    ...
    try {
      const summary = await callWithRetry(
        () => generateStructuredSummary(apiKey, settings, buildChunkPrompt(doc, chunk, settings), signal),
        signal,
        2,
      );
...
async function reducePartials(doc, partials, apiKey, settings, maxChunkTokens, signal) {
  ...
  if (partialsFitInContext(partials, maxChunkTokens)) {
    return callWithRetry(
      () => generateStructuredSummary(apiKey, settings, buildMergePrompt(doc, partials, settings), signal),
      signal,
      2,
    );
  }
  ...
  for (const batch of batches) {
    ...
    const merged = await callWithRetry(
      () => generateStructuredSummary(apiKey, settings, buildMergePrompt(doc, batch, settings), signal),
      signal,
      2,
    );
    reduced.push(merged);
