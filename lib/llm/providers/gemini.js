...
function geminiBaseUrl(settings) {
  const fallback = getProviderConfig('gemini').defaultBaseUrl;
  const base = (settings.providerUrl || fallback).replace(/\/$/, '');
 ...
 */
async function geminiGenerateStructuredSummary(apiKey, settings, prompt, signal) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 404) {
      if (body.includes('no longer available') || body.includes('not found')) {
        throw new Error('That AI model is no longer available. Open Settings and pick a current model.');
      }
    }
  ...
  if (!text) {
    throw new Error(
      finishReason ? `Gemini returned no text (${finishReason})` : 'Empty response from Gemini',
    );
  }
 ...
 * @param {LlmSettings} settings
 */
async function geminiTestConnection(apiKey, settings) {
  await ensureGeminiPermission();
...
const geminiProvider = {
  ...
  testConnection: geminiTestConnection,
  generateStructuredSummary: geminiGenerateStructuredSummary,
};
