...
const PRIORITY_STATUSES = /** @type {const} */ (['yes', 'no', 'unclear']);
const PRIORITY_KEYS = /** @type {const} */ ([
  'sellsOrSharesData',
  'dataForAdvertising',
  'thirdPartySharing',
  'hasRefundPolicy',
...
const UNCLEAR_PRIORITY = { status: 'unclear', answer: '' };

/**
 * @returns {import('../types.js').PriorityConcerns}
 */
function defaultPriorities() {
  return {
    sellsOrSharesData: { ...UNCLEAR_PRIORITY },
    dataForAdvertising: { ...UNCLEAR_PRIORITY },
    thirdPartySharing: { ...UNCLEAR_PRIORITY },
    hasRefundPolicy: { ...UNCLEAR_PRIORITY },
    hasCancellationPolicy: { ...UNCLEAR_PRIORITY },
    paymentsRefundable: { ...UNCLEAR_PRIORITY },
  };
}
...
function validatePriorities(raw) {
  ...
  /** @type {import('../types.js').PriorityConcerns} */
  const out = { ...defaults };
  ...
  for (const key of PRIORITY_KEYS) {
    ...
    const statusRaw = String(p.status ?? 'unclear').toLowerCase();
    const status = PRIORITY_STATUSES.includes(/** @type {typeof PRIORITY_STATUSES[number]} */ (statusRaw))
      ? statusRaw
      : 'unclear';
    out[key] = {
      status,
...
function validateSummary(data) {
  ...
  return {
    priorities: validatePriorities(obj.priorities),
