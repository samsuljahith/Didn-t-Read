...
async function ensureFetchPermission(url) {
  ...
  const err = new Error(
    `Need permission to fetch the linked policy from ${new URL(url).origin}.`,
  );
  err.code = 'NEEDS_ORIGIN_PERMISSION';
  err.policyUrl = url;
  throw err;
}
 ...
 * @returns {import('./types.js').ExtractedDoc}
 */
function buildExtractedFromHtml(html, url, meta) {
...
async function extractFromUrl(url, meta) {
  ...
  return buildExtractedFromHtml(await res.text(), url, meta);
}
